...
- **Settings** — a gear next to the bell opens a tabbed modal (Display · Account · Notifications · Privacy; only Display is implemented). **Display** exposes four per‑device preferences: **Map Quality**, **Map Base Style** (Dark / Light / Streets, all CARTO; dark default; tiles swap live), **Point Color** (the marker color — a rainbow of presets plus a color wheel for any custom hex; points re‑color live), and **Reduce Motion** (force‑disables animations/transitions, applied before first paint). All persist per device, unsynced to the account
