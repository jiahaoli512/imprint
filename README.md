# Imprint (v2.7) *Updated Jul 14, 2026*
...
- **Email** — Brevo (Sendinblue) transactional API for approval, signup/reset verification codes, contact‑form, friend‑request/accepted, and data‑export CSV emails
- **Auth** — JWT for users and admins; bcrypt password hashing; emailed 6‑char verification codes for signup and password reset; server‑side session revocation (`tokenVersion`) on password reset, in‑app password change, or a full "Log Out of All Devices"
...
- **Settings** — a gear next to the bell opens a tabbed modal (Display · Account · Notifications · Privacy; Notifications/Privacy are still "Coming soon"). Every setting row shows a "?" help icon (hover on desktop, tap‑to‑toggle on touch/native) instead of an always‑visible description. **Display** exposes six per‑device preferences: **Map Quality**, **Map Base Style** (Dark / Light / Streets, all CARTO; dark default; tiles swap live), **Point Shape** (Dot or Pin), **Point Color** (the marker color — brand + rainbow presets plus an always‑expanded HSV picker with drag / RGB / hex entry; points re‑color live), **Point Opacity** (a −100%…+100% slider; +100% fully opaque, −100% transparent, 0 = default), and **Reduce Motion** (force‑disables animations/transitions, applied before first paint) — all persist per device, unsynced to the account. **Account** exposes four self‑service actions: **Edit Profile** (jumps straight into edit mode on your own profile), **Change Password** (re‑enter your current password, distinct from the forgot‑password email‑code flow), **Export Your Data** (emails your location history + map markers as two CSV attachments to your account's own address, rather than a browser download), and **Log Out of All Devices** (revokes every session, including the current one, via a `tokenVersion` bump)
...
backend/
  src/
    routes/        # thin HTTP handlers (admin, users [an aggregator of
                   #   authRoutes/accountRoutes/profileRoutes], friends,
                   #   activity, waitlist, markers, locations, verification,
                   #   contact)
    services/      # business logic — user domain split by responsibility
                   #   (authService, passwordResetService, sessionService,
                   #   exportService, profileService, verificationService,
                   #   friendService, friendNotifications, activityService,
                   #   markerService, adminService, ...)
    ...
    utils/         # validate (generic string checks + name/username),
                   ...
                   #   cooldowns, email, csv (export CSV serialization), token
                   #   (JWT sign/verify seam), markerGeometry, names, httpError
frontend/
  src/
    ...
    features/settings/ # settings modal + per-device stores (SettingsModal,
                       #   Setting [shared help-icon row], DisplaySettings,
                       #   AccountSettings, useAccountExport,
                       #   useLogoutAllDevices, createSetting, reduceMotion)
...
Approval emails (on waitlist approval), signup/reset verification codes, friend‑request and friend‑accepted notifications, contact‑form submissions, and data‑export CSV attachments (Settings → Account → Export Your Data) are all sent through the Brevo transactional API (a single `sendEmail` seam behind a shared branded template).
...
- **Session revocation** — user tokens carry a `tv` claim checked against the account's `tokenVersion` on every authenticated request; a password reset, an in‑app password change, or "Log Out of All Devices" bumps the version, invalidating every previously‑issued token (including a stolen one)
...
- **Rate limiting** — a general API limiter, a strict limiter on login/registration/admin‑login, a dedicated (separate‑bucket) limiter on in‑app password changes, a cap on verification‑code requests, a friend‑request limiter (30/hour), a data‑export limiter (10/hour), and a dedicated cap on the public contact form
...
- **CSV‑injection‑safe export** — the waitlist CSV and the self‑service data‑export CSVs both escape quotes and neutralize formula triggers (`= + - @`); the data export is emailed to the account's own address (a self‑verified destination) rather than returned to the client, capped at 50,000 raw location points
